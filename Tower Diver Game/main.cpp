...
_timer *timer;               //Control Draw Screen Intervals
stack<_scene*> scenes;       //Hold all active scenes in a stack. Draw only the top one
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	if(!fullscreen)
        height -= 16;                   //Compensate for the Title Bar
					...
					int			nCmdShow)			// Window Show State
{
    ...
    timer = new _timer();                            //Create timer instance for draw control

    _blankScreen* bs = new _blankScreen(&scenes);     //Create a blank scene for catching QUIT
    scenes.push(bs);                                //Push it to bottom of the scene stack
    _titleScreen* ts = new _titleScreen(&scenes);     //Create the Title Screen
